...
static void SpeedTask(void *p_arg);
static void DirTask(void *p_arg);
...
static void MonitorTask(void *p_arg){
    ...
    static CPU_TS       ts;
	while(1){
		OSFlagPend(&speedDirFlags, DIR_FLAG | SPEED_FLAG, 0, OS_OPT_PEND_FLAG_SET_ANY | OS_OPT_PEND_BLOCKING, &ts, &err);
		if(mySpeed.speed > 75 || (mySpeed.speed > 45 && myDir.dir != NO_DIR)){
			OSFlagPost(&ledFlags, LED0_FLAG, OS_OPT_POST_FLAG_SET, &err);
		}
		else{
			OSFlagPost(&ledFlags, LED0_FLAG, OS_OPT_POST_FLAG_CLR, &err);
		}
		if(myDir.time_elapsed > 90 && myDir.dir != NO_DIR){
			OSFlagPost(&ledFlags, LED1_FLAG, OS_OPT_POST_FLAG_SET, &err);
		}
		else{
			OSFlagPost(&ledFlags, LED1_FLAG, OS_OPT_POST_FLAG_CLR, &err);
		}
        OSTimeDly( CAP_DELAY,                                        /*   1000 OS Ticks                                      */
                   OS_OPT_TIME_DLY,                             /*   from now.                                          */
                  &err);
...
static void LCDTask(void *p_arg){
	...
	static char myspeedStr[3];
	while(1){
		sprintf(myspeedStr, "%d", mySpeed.speed);
		RETARGET_WriteString("clear");
		RETARGET_WriteString(myspeedStr);
		RETARGET_WriteString(" MPH\n");

		//RETARGET_WriteString("\n\nYou are moving:\n");
		switch(myDir.dir){
		case HARD_LEFT:
		case SOFT_LEFT:
			RETARGET_WriteString("LEFT\n\n\t\t");
			RETARGET_WriteString(left_arrow);
			break;
		case SOFT_RIGHT:
		case HARD_RIGHT:
			RETARGET_WriteString("RIGHT\n\n\t\t");
			RETARGET_WriteString(right_arrow);
			break;
		case NO_DIR:
			RETARGET_WriteString("STRAIGHT\n\n\t\t");
			//RETARGET_WriteString(straight_arrow);
		}
		RETARGET_WriteString("                                                                             ");
        OSTimeDly( 2000,                                        /*   1000 OS Ticks                                      */
                   OS_OPT_TIME_DLY,                             /*   from now.                                          */
                  &err);
	}
}
